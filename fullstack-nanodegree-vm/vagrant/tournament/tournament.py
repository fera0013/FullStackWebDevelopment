...
def playerStandings():
    ...
    DB=connect()
    c=DB.cursor()
    c.execute("SELECT id, name, wins,matches FROM win_loss_table")
...
def swissPairings():
    ...
    DB=connect()
    c=DB.cursor()
    c.execute("SELECT id,name FROM win_loss_table ORDER BY wins")
    ranked_list = c.fetchall()
    DB.close()
    return [(ranked_list[i][0],ranked_list[i][1], ranked_list[i+1][0],ranked_list[i+1][1]) for i in xrange(0, len(ranked_list), 2)]
